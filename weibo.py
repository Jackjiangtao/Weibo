...
class Weibo(object):
    def __init__(self,
        ...
        self.user_id = '1669879400'  # 用户id,如昵称为"Dear-迪丽热巴"的id为'1669879400'
    ...
    def print_novel_coronavirus_weibo(self, weibo):
        """打印新型肺炎微博"""
        isAppend =True
        for WeiboIndex in self.novel_coronavirus_weibo:
             if WeiboIndex['text'] == weibo['text'] :
                   isAppend=False

        keyword=[u"陕西新增",u"新型冠状病毒感染的肺炎确诊病例"]

        if isAppend:
            for word in keyword:
                if weibo['text'].find(word)  != -1 :
                  self.novel_coronavirus_weibo.append(weibo)
    ...
    def print_weibo(self, weibo):
        ...
        self.print_novel_coronavirus_weibo(weibo)
        #print('-' * 120)
    ...
    def start(self, user_id_list):
        ...
        try:
            for user_id in user_id_list:
                ...
                weibotext =[]
                for weibo in self.novel_coronavirus_weibo:
                    if weibo['text'] not in weibotext:
                        weibotext.append(weibo['text'])
                        print(weibo['text'])
...
def main():
    try:
        ...
        since_date = '2020-1-23'  # 起始时间，即爬取发布日期从该值到现在的微博，形式为yyyy-mm-dd
